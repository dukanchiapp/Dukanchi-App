...
> Last updated: 2026-06-15 | Session 128.51 | 🏁 **PHASE 6.2 COMPLETE — final coverage hardening: store.service (26.56% → 98.43%) + user.service (30.3% → 100%). +54 tests. NO bugs. Coverage wave finished — all 14 high-blast-radius services covered.** (PR [#215](https://github.com/dukanchiapp/Dukanchi-App/pull/215) merged `3990029`; no deploy this session.) **Tests:** 594 → **648** (+54, zero pre-existing regressed). 28 store.service + 26 user.service. **9 marquee tests** — all PASSED. **store.service.ts ALLOWLIST defense MARQUEE:** crafted body with ownerId/role/isBlocked/adminPassword/kycStatus → silently DROPPED before prisma.update; only the 21 allowlist fields propagate (defense against attacker-crafted IDOR / role-escalation via crafted PATCH body). **store.service.ts role-visibility MARQUEE:** customer viewer → visibleRoles=[retailer] only; B2B viewer (retailer/supplier) → all 4 B2B roles. **store.service.ts soft-delete cascade MARQUEE:** getStoreById WHERE owner.deletedAt:null → deleted-owner stores return null (controller 404). **store.service.ts self-notification GUARD MARQUEE:** toggleFollow caller===owner → notification.create + socket emit SKIPPED (don't notify yourself); notification failure best-effort (Rule B-compliant). **user.service.ts FIELD-SCOPE MARQUEE:** getUserProfile select projects ONLY {id, name, role, email} — explicit negative assertions that phone/password/kycDocumentUrl/kycSelfieUrl/deletedAt are NOT in the select set (defense against field-explosion regression). **user.service.ts P2002 → opaque error MARQUEE:** Prisma unique-constraint error → "This phone number is already in use" (raw P2002 code MUST NOT leak); non-P2002 re-thrown unchanged (preserves Sentry visibility). **user.service.ts WHERE userId IDOR scope MARQUEE:** all 5 read methods (followed/saved/reviews/searchHistory/locations) explicitly scoped to userId; cross-user USER_B param → only USER_B in WHERE, never USER_A. **user.service.ts 4 cap sentinels MARQUEE:** PER_USER_FOLLOWED_CAP=5000 / SAVED_ITEMS=5000 / REVIEWS=5000 / LOCATIONS=1000 — each fires Sentry.captureMessage on exact-cap hit; below cap → NO Sentry fire (explicit positive+negative assertions). **D2 anonymize policy preserved:** getSavedItems surfaces deleted-author posts (Session 88 policy preserved end-to-end). **Coverage:** store.service.ts **0% → 98.43% lines** (Stmts 98.61 / Branches 94.28 / Funcs **100 (22/22)** · 1 uncovered: line 152 re-throw cosmetic). user.service.ts **0% → 100% lines** (33/33, all branches, all 11 funcs). **Gates:** npm test 648/648 (was 594; +54) · npm run typecheck 3 projects clean (Rule G) · npm run build clean. **Coverage wave grand total (Phase 4 + 5 + 6.1 + 6.2, 6 sessions, 14 services): +434 tests across 14 high-blast-radius services. Thread-cumulative: 199 → 648. All 14 services ≥89% lines; 7 of 14 ≥95%.** Session 128.49 diagnostic identified store.service + user.service as the last 2 gaps; both now closed. **The coverage wave is COMPLETE.** Rule A: N/A (test-only). Rule C: N/A. Rule E: N/A (no deploy).
> Last updated: 2026-06-15 | Session 128.50 | 🧹 **DUPLICATE REACT /landing ROUTE REMOVED — public/landing.html (Session 128.23 rich orange design) is now the sole landing experience. Fly v113 → v114, −37 kB bundle.** (PR [#214](https://github.com/dukanchiapp/Dukanchi-App/pull/214) pending; deploy COMPLETE + Rule E 5-smoke battery green.) **Bug:** Founder verified v113 → logged-in users hitting /landing got the React-SPA LandingPage.tsx ("Ab Aapki Local Market Aapke Phone Par"), while logged-out users got the static public/landing.html ("Sab milega jo aas-paas hai" rich orange). Two designs at the same URL depending on auth — confusing UX. **Fix:** deleted `src/pages/LandingPage.tsx` (557 lines) + removed lazy import + `<Route path="/landing">` from src/App.tsx. The 2 remaining /landing refs (navigate at App.tsx:148 + `<a href>` at App.tsx:189) are CORRECT — they trigger a full page load, the browser hits GET /landing, Express sendFile public/landing.html serves the static page. React-SPA no longer intercepts that route. **Phase 0 audit:** grep'd all LandingPage refs (3 component hits — all removed), `<Route path="/landing">` (1 — removed), `navigate(/landing)` + `<a href=/landing>` (2 — KEPT as correct), `<PageMeta>` for landing (0 — no orphan), test imports of LandingPage (0 — no test cleanup). 3+3 ref baseline matched exactly. Zero deviations. **Bot-render unchanged:** Session 128.40 botRenderLanding middleware mounted on `/` AND `/landing` continues to handle bot/unfurler UAs (WhatsApp/Slack/Twitter/Telegram/Facebook/Googlebot) before falling through to the static handler. Smoke 3 confirms WhatsApp UA still gets the OG meta block. **Bundle delta:** LandingPage-Bo08goAA.js eliminated — **−37.05 kB raw / −7.57 kB gzip / −58.49 kB sourcemap**. One less lazy chunk in SW precache manifest. **Gates:** `npm test` 534/534 (unchanged — no test imports LandingPage) · `npm run typecheck` 3 projects clean (Rule G) · `npm run build` clean, no LandingPage chunk in dist/. **Rule E smoke battery (v114, all green):** /health 200 application/json ✅ · /landing Chrome → `<!DOCTYPE html><html lang="en" data-theme="yellow">` (static landing.html — `data-theme="yellow"` is unique to public/landing.html, proves NOT the SPA index.html) ✅ · /landing Chrome `id="root"` count = 0 ✅ (NOT the SPA shell) · /landing WhatsApp → `<title>Dukanchi — apna bazaar, apni dukaan</title>` + full OG/Twitter meta (botRenderLanding intact, Session 128.40 path preserved) ✅ · / Chrome → static landing.html (Session 128.12 behavior unchanged) ✅ · / Chrome HTTP 200 ✅. **Awaiting founder browser smoke:** load dukanchi.com/landing in incognito → "Sab milega jo aas-paas hai" rich orange design · load /landing in authenticated tab → SAME design (NOT the old "Ab Aapki Local Market" SPA design) · logout from inside the app → lands on /landing → SAME design. If any tab shows the OLD design → likely stale service-worker cache, hard-refresh + screenshot. Rule A: yes — one React route removed (/landing). Express route unchanged. No user-visible URL change. Rule C: N/A. Rule D: N/A (web-only). Rule F: N/A.
