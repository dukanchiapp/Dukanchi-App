...
> Last updated: 2026-06-15 | Session 128.48 | 🚨 **BUG FIXES FROM FOUNDER v112 PROD VERIFICATION — CSP places.googleapis.com (HIGH, location autocomplete blocked) + Messages spacing (MEDIUM, cramped header). Fly v112 → v113.** (PR [#212](https://github.com/dukanchiapp/Dukanchi-App/pull/212) pending; deploy COMPLETE + Rule E smoke battery green.) **BUG #1 (HIGH):** Founder verified v112 (this morning's CSP enforce flip) → "Choose location" Places autocomplete returned ZERO_RESULTS for valid queries like "rohini". Root cause confirmed via CSP header inspection: @googlemaps/places library makes background fetches to `places.googleapis.com`, but v112's CSP enforce only allowed `maps.googleapis.com` + `maps.gstatic.com` in scriptSrc + connectSrc. The new host was silently blocked — autocomplete getPlacePredictions callback returned ZERO_RESULTS regardless of query. **Fix:** Added `https://places.googleapis.com` to BOTH `scriptSrc` AND `connectSrc` in src/app.ts (adjacent to existing maps.googleapis.com entries, with comment marking session origin). **Why both:** script-src — the @googlemaps/places library (post-2024 surface) loads sub-scripts from the host; connect-src — AutocompleteService XHR + PlacesService.getDetails fetches hit the host directly. **Audit:** Grepped all `googleapis.com`/`gstatic.com` references in src/. No other client-side gaps. The 4 Gemini callsites (search.service / ask-nearby.service / bulkImport.service / geminiEmbeddings.ts) use `generativelanguage.googleapis.com` from the Node process — server-side only, never reaches browser, no CSP entry needed. **BUG #2 (MEDIUM):** Messages.tsx had cramped spacing between yellow header band, search pill, tabs, and first conversation card. 5 spacing-only edits (no color/font/layout changes): header padding bottom 0 → 16, title row marginBottom 12 → 14, tabs marginTop 12 → 14, loading skeletons + conversation list padding "8px 16px" → "14px 16px 10px". **Regression guard:** `src/__tests__/csp-places.test.ts` (+3 tests, source-text scan pattern mirroring csp-enforce.test.ts — avoids booting Sentry/Redis/Prisma). Asserts scriptSrc + connectSrc contain places.googleapis.com AND maps.googleapis.com preserved in both (regression safety net for the Maps SDK callsites). **Tests:** 531 → **534** (+3, zero pre-existing regressed). `npm run typecheck` 3 projects clean (Rule G — never `npx tsc --noEmit`). `npm run build` clean. **Rule E smoke battery (post-v113):** /health → 200 application/json ✓ · CSP script-src verbatim includes `https://places.googleapis.com` ✓ · CSP connect-src verbatim includes `https://places.googleapis.com` ✓ · maps.googleapis.com preserved in scriptSrc + imgSrc + connectSrc ✓ · Chrome UA → / → 200 ✓. **Awaiting founder browser smoke:** load dukanchi.com → DevTools Console → click "Change" → search "rohini" → predictions should populate · Messages page visual breathing room confirm · console ZERO CSP errors. If any other Google subdomain CSP violation surfaces, follow-up PR. Rule A: N/A (no URL change). Rule C: N/A (no Capacitor change). Rule D: N/A (web CSP only). Rule F: N/A (no write-path smokes). Phase E queue impact: zero new items added.
