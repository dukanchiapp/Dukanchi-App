...
> Last updated: 2026-06-17 | Session 128.56 | 🐛 **MESSAGES QUERIES — 3 BUGS FIXED — Bug 3 (backend data path): getPendingRequests Prisma select was missing the images field, retailers saw empty arrays even when customers attached photos · Bug 4 (UI): 60→96 px previews with border/shadow/wrap · Bug 5 (A11Y): tab text contrast on yellow header — WCAG AA fix. Fly v118 → v119.** (PR [#225](https://github.com/dukanchiapp/Dukanchi-App/pull/225) merged `697e44c`; deploy COMPLETE + smokes green.) **Bug 3 (DATA, real backend bug):** Prisma schema has AskNearbyRequest.images String[] @default([]) (line 369) and sendAskNearby PERSISTS images correctly — but getPendingRequests select clause (lines 282-293) was missing `images: true`, so the API never surfaced the field to the frontend. Messages.tsx:87 read `item.request.images || []` and always fell through to []. Added `images: true` to the request.select block. IDOR test asserts WHERE+take only (not select-include shape) → non-breaking. **Bug 4 (UI):** Queries card preview hardcoded width:60 height:60 with no border/shadow → bumped to 96×96 with 1px #E5E7EB border + soft shadow + flexWrap so multiple images wrap gracefully on narrow screens. Added alt="Attached photo N" + loading="lazy" for a11y. Why 96 vs Search modal's 80: this is a retailer-facing decision surface (review stock request → yes/no decision) — bigger previews aid faster decisions; Search modal's 80 is customer-composing context. **Bug 5 (A11Y, WCAG AA):** Yellow Messages header had rgba(255,255,255,0.6) inactive + white active tab text — white-on-yellow at 60% opacity fails contrast. Switched to #111827 (near-black) active + rgba(0,0,0,0.55) inactive + #111827 underline. Dark-on-yellow is the standard high-contrast pairing (think traffic signs). **Gates:** npm test 648/648 (unchanged — IDOR test asserts WHERE+take only) · npm run typecheck 3 projects clean (Rule G) · npm run build clean. **Rule E smoke battery (v119, all green):** /health → 200 db:up redis:up ✅ · fly logs error scan → ZERO error/fatal/crash ✅ · Messages chunk hash → Messages-8OYukFw1.js (rebuilt) ✅ · prod chunk content scan: width:96 = 1 · #111827 (active + underline) = 4 · rgba(0,0,0,0.55) (inactive tab) present · rgba(255,255,255,.6) (old white-on-yellow) = 0 ✅. **Awaiting founder visual smoke on v119:** (1) customer attaches photo via Search → retailer's Queries card shows the image at 96×96; (2) Chats/Queries tabs read clearly as dark text on yellow header. Rule A: N/A (existing endpoint, additional field surfaced — no route change). Rule C: N/A. Rule D: N/A.
