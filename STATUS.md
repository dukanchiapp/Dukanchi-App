...
> Last updated: 2026-06-17 | Session 128.57 | 🖼️ **MESSAGES LIGHTBOX — full-screen viewer when retailer taps a customer's attached photo on Queries card. Industry-standard pattern (WhatsApp / Instagram). Fly v119 → v120.** (PR [#227](https://github.com/dukanchiapp/Dukanchi-App/pull/227) merged `07baf6b`; deploy COMPLETE + smokes green.) **Was missing affordance** after PR #225 shipped clickable 96×96 photo previews — tap did nothing. **Founder-spotted on v119.** **Implementation:** inline component in src/pages/Messages.tsx — **no new dependency** (avoided react-image-lightbox / yet-another-react-lightbox bloat). One useState<string|null> for the open URL + one useEffect for ESC + body scroll lock. **Three close paths:** (1) backdrop tap with zoom-out cursor; (2) X button top-right (44×44 touch target — meets Apple HIG / Material a11y guidelines); (3) ESC key. **Body scroll lock** while open. **e.stopPropagation** on the image itself so tapping the photo doesn't close the modal under the user. **A11Y:** role="dialog" + aria-modal="true" + aria-label="Attached photo viewer" on overlay; aria-label="View attached photo N fullscreen" on each thumbnail trigger; aria-label="Close photo viewer" on the X button; thumbnails now in <button type="button"> wrapper for proper keyboard activation. Original 96×96 thumbnail styling from #225 preserved exactly (border + shadow), now inside an unstyled transparent button. **Gates:** npm test 648/648 (unchanged — no test touches Messages JSX) · npm run typecheck 3 projects clean (Rule G) · npm run build clean. **Rule E smoke battery (v120, all green):** /health → 200 db:up redis:up ✅ · fly logs error scan → ZERO error/fatal/crash ✅ · Messages chunk hash v119 Messages-8OYukFw1.js → v120 Messages-6SrjtOXj.js (rebuilt, 20,122 bytes) ✅ · prod chunk content scan: aria-modal=1 · Escape (ESC handler)=1 · "Close photo viewer" (X aria)=1 · "Attached photo viewer" (overlay aria)=1 · body.style.overflow (save+apply+restore scroll lock)=3 ✅. **Awaiting founder visual smoke on v120 (retailer flow):** (1) Open Messages → Queries tab; (2) tap an attached customer photo → fullscreen viewer opens with dark overlay; (3) backdrop tap closes; (4) X button closes; (5) ESC key closes (desktop); (6) tapping the image itself does NOT close the modal. Rule A: N/A (no route change). Rule C: N/A. Rule D: N/A.
