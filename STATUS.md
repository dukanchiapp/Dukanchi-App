...
> Last updated: 2026-06-17 | Session 128.54 | 🎨 **DOODLE IS NOW THE IMMEDIATE COVER FALLBACK — logoUrl-blur banner layer removed. Fly v116 → v117.** (PR [#221](https://github.com/dukanchiapp/Dukanchi-App/pull/221) merged `24334ce`; deploy COMPLETE + Rule E smokes green.) **Bug:** Founder verified via v116 production screenshot — stores without `coverUrl` but WITH `logoUrl` were rendering BLURRED-LOGO banners instead of the Dukanchi doodle. The intermediate logoUrl-blur fallback (introduced before #217) was overriding the doodle for any store with a logo. **Founder spec from #217 was "doodle for ALL stores without coverUrl, regardless of old/new"** — the logoUrl-blur layer was an unintended override. **Fix:** removed 8 lines total — the middle ternary branch in src/pages/Profile.tsx (line 627-628, blur 14px / scale 1.2) and src/pages/StoreProfile.tsx (line 333-338, blur 14px / scale 1.15). **Fallback chain (post-fix):** Layer 1: `store.coverUrl` → uploaded cover image · Layer 2: doodle pattern → ALWAYS when no coverUrl. **Was 3 layers (cover → logoUrl-blur → doodle), now 2 layers (cover → doodle).** **Small circular 92px logo elsewhere on page UNTOUCHED** — different JSX subtree (Profile.tsx line 662, StoreProfile ~line 407), uses logoUrl for the avatar circle, not the 160px banner. **Gates:** npm test 648/648 (unchanged — no test references logoUrl-blur banner) · npm run typecheck 3 projects clean (Rule G) · npm run build clean. **Rule E smoke battery (v117, all green):** /health → 200 db:up redis:up ✅ · /store-cover-doodle.png HEAD → HTTP/2 200 ✅ · fly logs error scan → ZERO error/fatal/crash entries ✅ · Profile chunk hash v116 Profile-CW-mjaRw.js → v117 Profile-7_BN5S5g.js (rebuilt) ✅ · StoreProfile chunk hash v116 StoreProfile-DVLl0HvN.js → v117 StoreProfile--Sq3HOnA.js (rebuilt) ✅ · prod-bundle content scan: both Profile + StoreProfile chunks show blur(14px) hits = 0 (logoUrl-blur GONE) and store-cover-doodle hits = 1 (doodle code intact). **Awaiting founder visual smoke on v117:** store with no coverUrl but WITH logoUrl should now show the Dukanchi doodle banner, NOT the blurred-logo banner. The small 92px circular logo on the same page is unchanged. Rule A: N/A (no route change). Rule C: N/A. Rule D: N/A. Rule F: N/A.
