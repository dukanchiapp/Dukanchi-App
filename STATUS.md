...
> Last updated: 2026-06-16 | Session 128.52 | 🎨 **DOODLE COVER FALLBACK — Profile + StoreProfile now use tiled दुकानची doodle pattern as the layer-3 cover fallback (was: plain gradient).** (PR [#217](https://github.com/dukanchiapp/Dukanchi-App/pull/217) merged `c4fe0b3`; no deploy this session — pending founder visual smoke.) **What changed:** layer-3 of the cover fallback chain in `src/pages/Profile.tsx` (line 630) + `src/pages/StoreProfile.tsx` (line 340) — when both `coverUrl` AND `logoUrl` are null, the banner now renders the tiled Dukanchi doodle PNG on a cream paper-tone backdrop (#FFF8EA) instead of the previous plain `var(--b-grad)` gradient. Layers 1 (coverUrl image) + 2 (blurred logoUrl) unchanged. **Asset:** `public/store-cover-doodle.png` (178,583 bytes / ~174 KB). **Bundle delta:** precache 79 → 80 entries / 2760.57 → 2935.26 KiB (+174.69 KiB raw; PNG already lossy-compressed so gzip≈raw). **Visual choices:** backgroundSize 300px auto (preserves portrait aspect at tasteful tile density), backgroundColor #FFF8EA (matches cream paper-tone so tile seams invisible), role="img" + aria-label="Dukanchi doodle pattern" (a11y parity). **Phase 2 (cover-picker UI upgrade) DEFERRED:** the cover-upload UI is NOT in Profile.tsx — it lives in `src/components/dashboard/StoreFormFields.tsx` (lines 91-107, used by RetailerDashboard). Existing UI has only "Change Cover" / "Add Cover Photo" — no "remove" / "default" option. Task spec said to STOP and report rather than speculate-add UI. The fallback naturally activates whenever coverUrl is null (account creation path; or a future "remove cover" / "use default doodle" button). Follow-up PR candidate. **Tests:** 648/648 green (unchanged — no tests touch the banner fallback layer). **Gates:** typecheck 3 projects clean (Rule G) · build clean · precache delta verified. **Awaiting:** Founder visual smoke at http://localhost:5173/ on both Profile and StoreProfile pages for stores without coverUrl. Production deploy decision pending founder approval. Rule A: cosmetic-only (no route added/removed). Rule C: N/A. Rule D: N/A. Rule E: not invoked this session.
