...
> Last updated: 2026-05-28 | Session 111 | ✅ **CSP: REPORT-ONLY (safe launch state)** (Fly v29, PR #76, commit `bd0f974`). Rolled back the S108 enforce-flip because the SW-precached `index.html` baked the stale enforced CSP header into legacy PWA installs — server-side fixes (including the S110 `data:` / `blob:` hotfix) couldn't propagate without cache churn, leaving image upload broken. `data:` / `blob:` in `connect-src` RETAINED for a clean re-enforce later. Image moderation (S109b) + ND-A1 push handler unaffected. Task 5 CSP enforce flip — RE-OPENED as fresh-day task (SW network-first nav + flow audit + SW versioning ceremony).
> Single-page snapshot. History → SESSION_LOG.md. Decisions → DECISIONS.md.
>
> **Pending:** Founder hard-refresh / SW unregister + re-test image upload from web + APK — expect 200 + url, no CSP block (report-only blocks nothing). Watch `dukanchiapp@gmail.com` Sentry for new `Content-Security-Policy-Report-Only` violations over next 24h — these are the audit signal for the re-enforce checklist. "Image moderation rejection" (info) + "upload.moderation.gemini_error" (warning) rates still being monitored from S109b. Tier 5B (E2E tests) + Tier 5C (perf smoke) queued. Re-enforce CSP = fresh-day task: (a) SW network-first navigation for index.html, (b) exercise all real user flows in report-only, (c) SW version bump ceremony, (d) flip. Checkpoint 4 (device verify): ND-D2-APK-SMOKE + browser PWA push end-to-end still pending.
...
Production HEAD: `050dc62` → `eda3a4d` (PR #43) → `9475c90` (PR #44) → `cd18d1a` (PR #47 npm audit fix) → `a8b64bf` (PR #70 CSP enforce, Fly v26) → `4091dbf` (PR #72 image moderation, Fly v27) → `31c5232` (PR #74 CSP connect-src data:/blob: hotfix, Fly v28) → **`bd0f974`** (PR #76 CSP reverted to report-only, Fly v29); APK signed release built locally (not on `main`)
