...
> Last updated: 2026-06-17 | Session 128.55 | 🐛 **ASK NEARBY ROOT-CAUSE FIX — frontend was POSTing FormData field "image", server expects "file" (multer upload.single("file")). Every ask-nearby image upload was 400ing silently → "Network error" toast hid the real cause. Plus 40→80px thumbnails + Hinglish error messages. Fly v117 → v118.** (PR [#223](https://github.com/dukanchiapp/Dukanchi-App/pull/223) merged `8e456cc`; deploy COMPLETE + smokes green.) **Discovery during RECON:** Audited all 9 frontend /api/upload callsites — KYCForm, BulkUpload, PostsGrid×2, RetailerDashboard×3, Profile, Chat ALL use field "file"; only Search.tsx:408 used "image" (copy-paste bug from when ask-nearby was wired in). Multer silently rejected every ask-nearby image with 400 "No file uploaded"; outer catch surfaced generic "Network error, dobara try karo" toast. **Bug 2 fixes (3 layers):** (a) ROOT CAUSE — formData.append("image" → "file") so multer accepts the upload; (b) pre-flight 10MB size guard with Hinglish message "Image N bahut badi hai (10 MB se zyada)…"; (c) server error body parsing — if !res.ok, parse JSON error/message + surface via toast (fallback "Image upload fail hua (status)" for non-JSON); (d) outer catch now constructs "Request bhejne mein dikkat: <err.message>" instead of generic "Network error". Two distinct Sentry contexts: search.askNearby.imageUpload (with imageCount) + search.askNearbySend. **Bug 1 fix (visual hierarchy):** Add-image button 40×40 (SVG 20) → 80×80 (SVG 28); thumbnails 40×40 → 80×80; close X button 12px → 22×22 hit target; added aria-label + display:block on img. **Diagnostic signal:** Prod fly logs for last 4h surfaced ZERO upload/ask-nearby entries before the fix — meaning the request never reached the server. Consistent with the root-cause being client-side. **Gates:** npm test 648/648 (unchanged) · npm run typecheck 3 projects clean (Rule G) · npm run build clean. **Rule E smoke battery (v118, all green):** /health → 200 db:up redis:up ✅ · fly logs error scan → ZERO error/fatal/crash ✅ · Search chunk hash → Search-C1qhkzcl.js (35,810 bytes, rebuilt) ✅ · prod chunk content scan: "Request bhejne mein dikkat" = 1 · "bahut badi hai" = 1 · 10485760 (10 MB minified literal) = 1 · `append("file"` = 1 · `append("image"` = 0 (old bug GONE) · width:80,height:80 = 2 (button + thumbnail) ✅. **Awaiting founder visual smoke on v118:** (a) thumbnails + Add-image button should be 80×80; (b) attach image + click Send Request → should succeed OR show specific error message (e.g. "Image 1 bahut badi hai…" or "Image upload fail hua (413)"), never the generic "Network error, dobara try karo". Rule A: N/A (no route change). Rule C: N/A. Rule D: N/A.
