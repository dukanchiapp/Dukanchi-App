...
> Last updated: 2026-06-16 | Session 128.53 | 🔒 **CRITICAL UX FIX — Signup form autofill bug: browsers were pre-filling /signup with the founder's saved admin login credentials because phone+password fields lacked autoComplete attributes.** (PR [#219](https://github.com/dukanchiapp/Dukanchi-App/pull/219) merged `629141c`; no deploy this session — pending founder visual smoke.) **Bug:** Chrome/Safari heuristically match any password input on the domain to saved logins regardless of context. Visiting /signup auto-populated phone+password with the founder's admin creds from the last login. **Fix:** 4 attribute additions on src/pages/Signup.tsx (zero logic changes): `<form autoComplete="off">` wrapper · name input `autoComplete="name"` · phone input `autoComplete="off"` (new-account ID, not a returning login) · password input `autoComplete="new-password"` (spec-correct for signup — browsers offer "Suggest strong password" rather than autofilling saved ones). **Login.tsx UNCHANGED** — autofill UX preserved for returning users (verified via grep: zero autoComplete attributes touched in Login.tsx). **Scope note:** Signup form has 3 inputs (name/phone/password) — NO Confirm Password field exists. Per task spec ("do not invent UI"), did NOT add a 4th input. The autofill bug is fully fixed by the 3 existing inputs + form-wrapper attribute. If a confirm-pw field is added later, it should also carry autoComplete="new-password". **Tests:** 648/648 green (unchanged — no tests touch signup input attributes). **Gates:** typecheck 3 projects clean (Rule G) · build clean. **Awaiting:** Founder visual smoke at http://localhost:5173/signup in fresh Chrome window — all 3 inputs should appear EMPTY by default; browser should NOT autofill phone/password with saved admin creds; password field when clicked may offer "Suggest strong password" (CORRECT new-password behavior). **Deferred — P2 follow-up before retailer onboarding window:** (1) createAdmin.ts security cleanup — remove from working tree, add to .gitignore, scrub git history via filter-repo + force-push (after stakeholder warning); (2) admin password rotation — rotate the credential that was auto-filling once history is scrubbed. Both = ONE bundled security-hardening PR. Rule A: N/A (no route change). Rule C: N/A. Rule D: N/A. Rule E: not invoked this session.
