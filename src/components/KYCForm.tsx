...
export default function KYCForm({ onComplete, onLogout, onBack }: { onComplete: () => void, onLogout: () => void, onBack?: () => void }) {
  ...
  return (
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen pb-20">
      ...
      <main className="p-4">
        <div className="bg-[#FFF6DD] rounded-xl p-4 mb-4 border border-[#FFE0CC] flex items-start">
          <Building className="mr-3 flex-shrink-0 mt-0.5 text-[#C77E00]" size={20} />
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-[#C77E00] focus:ring-2 focus:ring-[#FFE6B3] transition-all outline-none text-sm font-medium text-gray-900"
                />
              </div>
              ...
              <div>
                ...
                {selfie ? (
                  ...
                  <label className="relative border border-dashed border-[#FFB89E] bg-[#FFF6DD]/50 rounded-xl h-32 flex flex-col items-center justify-center cursor-pointer hover:bg-[#FFF6DD] transition-colors group overflow-hidden">
                    {uploadingField === 'selfie' ? (
                      <Loader2 size={24} className="text-[#C77E00] mb-2 animate-spin" />
                    ) : (
                      <>
                        <Camera size={24} className="text-[#C77E00] mb-2 group-hover:scale-110 transition-transform" />
                        <span className="text-xs text-[#C77E00] font-bold">Open Camera</span>
                      </>
                    )}
                ...
                {storePhoto ? (
                  ...
                  <label className="relative border border-dashed border-[#FFB89E] bg-[#FFF6DD]/50 rounded-xl h-32 flex flex-col items-center justify-center cursor-pointer hover:bg-[#FFF6DD] transition-colors group overflow-hidden">
                    {uploadingField === 'storePhoto' ? (
                      ...
                      <>
                        <Camera size={24} className="text-[#C77E00] mb-2 group-hover:scale-110 transition-transform" />
                        <span className="text-xs text-[#C77E00] font-bold">Capture Storefront</span>
                      </>
                    )}
                ...
                {gstBill ? (
                  ...
                  <label className="relative border border-dashed border-[#FFB89E] bg-[#FFF6DD]/50 rounded-xl h-32 flex flex-col items-center justify-center cursor-pointer hover:bg-[#FFF6DD] transition-colors group overflow-hidden">
                    {uploadingField === 'gstBill' ? (
                      ...
                      <>
                        <Camera size={24} className="text-[#C77E00] mb-2 group-hover:scale-110 transition-transform" />
                        <span className="text-xs text-[#C77E00] font-bold">Scan Document</span>
                      </>
                    )}
