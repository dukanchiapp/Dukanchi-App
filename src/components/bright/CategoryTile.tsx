import type { ReactNode } from 'react';

/* ── Session 128.13 — Bright Skin CategoryTile primitive ────────────────────
   Ported from dukanchi-bright-skin/react-stubs/CategoryTile.jsx AS-IS.

   The `.b-clay` class (in bright-skin.css) handles the 3D glossy clay look:
     - inset highlight + soft drop shadow
     - ::after sheen pseudo-element
     - press scale + lift on :active
   ─────────────────────────────────────────────────────────────────────────── */

interface BrightCategoryTileProps {
  emoji: string;
  label: string;
  bg?: string;
  onClick?: () => void;
}

export default function CategoryTile({ emoji, label, bg = 'var(--b-tint)', onClick }: BrightCategoryTileProps) {
  return (
    <button
      onClick={onClick}
      className="b-tap"
      style={{
        display: 'flex',
        flexDirection: 'column',
        alignItems: 'center',
        gap: 8,
        background: 'none',
        border: 'none',
        cursor: 'pointer',
        padding: 0,
      }}
    >
      <span className="b-clay" style={{ width: 58, height: 58, background: bg }}>
        <span className="b-clay-emoji" style={{ fontSize: 26 }}>{emoji as ReactNode}</span>
      </span>
      <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--b-ink)', textAlign: 'center' }}>{label}</span>
    </button>
  );
}

/* Reference data — the 11 production categories with their tint rotation. */
export const CATEGORIES = [
  { label: 'Food',        emoji: '🍕', bg: '#FFE9D6' },
  { label: 'Electronics', emoji: '📱', bg: '#E8F0FE' },
  { label: 'Fashion',     emoji: '👕', bg: '#E8F0FE' },
  { label: 'Grocery',     emoji: '🛒', bg: '#E8F5E9' },
  { label: 'Beauty',      emoji: '💄', bg: '#FDECEA' },
  { label: 'Health',      emoji: '💊', bg: '#E8F5E9' },
  { label: 'Jewellery',   emoji: '💍', bg: '#FFF1EA' },
  { label: 'Home',        emoji: '🏠', bg: '#E8F0FE' },
  { label: 'Books',       emoji: '📚', bg: '#E8F5E9' },
  { label: 'Auto',        emoji: '🚗', bg: '#FDECEA' },
  { label: 'Services',    emoji: '🛠️', bg: '#FFE9D6' },
] as const;
